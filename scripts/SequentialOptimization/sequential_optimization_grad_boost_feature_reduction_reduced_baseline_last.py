...
def main_loop(impute_type, n_neighbors, timestamp, model_type, method_key, baseline_window, window_size, stride, classifier_type,
            ...
            accuracy_gb, precision_gb, recall_gb, f1_gb, specificity_gb, g_mean_gb = (
                classify_ensemble_with_gradboost(x_train, x_test, y_train, y_test, random_state,
                                                 save_folder=os.path.join(
                                                     "../ModelSave/SequentialOptimizationFeatureReduction", timestamp,
                                                     method_key), retrain=train_class))

        performance_metric_summary = single_classifier_performance_summary(accuracy_gb, precision_gb, recall_gb, f1_gb, specificity_gb, g_mean_gb)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'ridge':
        ...
        for n in range(len(percentile_threshold)):
            ...
            if classifier_type == 'all' or classifier_type == 'EGB':
                accuracy_gb, precision_gb, recall_gb, f1_gb, specificity_gb, g_mean_gb = (
                    classify_ensemble_with_gradboost(x_train_ridge[n], x_test_ridge[n], y_train, y_test, random_state,
                                                     save_folder=os.path.join(
                                                         "../ModelSave/SequentialOptimizationFeatureReduction",
                                                         timestamp,
                                                         method_key), retrain=train_class))

            performance_metric_summary[n] = single_classifier_performance_summary(accuracy_gb, precision_gb, recall_gb,
                                                                               f1_gb, specificity_gb, g_mean_gb)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'mrmr':
        ...
        if classifier_type == 'all' or classifier_type == 'EGB':
            accuracy_gb, precision_gb, recall_gb, f1_gb, specificity_gb, g_mean_gb = (
                classify_ensemble_with_gradboost(x_train_mrmr, x_test_mrmr, y_train, y_test, random_state,
                                                 save_folder=os.path.join(
                                                     "../ModelSave/SequentialOptimizationFeatureReduction", timestamp,
                                                     method_key), retrain=train_class))

        performance_metric_summary = single_classifier_performance_summary(accuracy_gb, precision_gb, recall_gb,
                                                                           f1_gb, specificity_gb, g_mean_gb)
...
if __name__ == "__main__":
    ...
    feature_reduction_methods = ['pca', 'performance', 'none']
