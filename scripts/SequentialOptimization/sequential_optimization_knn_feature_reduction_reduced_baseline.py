...
def main_loop(impute_type, n_neighbors, timestamp, model_type, method_key, baseline_window, window_size, stride, classifier_type,
        ...
        if classifier_type == 'all' or classifier_type == 'KNN':
            accuracy_knn, precision_knn, recall_knn, f1_knn, specificity_knn, g_mean_knn = (
                classify_knn(x_train, x_test, y_train, y_test, random_state,
                             save_folder=os.path.join("../ModelSave/SequentialOptimizationFeatureReduction",
                                                      classifier_type,
                                                      timestamp, method_key), retrain=train_class))

        performance_metric_summary = single_classifier_performance_summary(accuracy_knn, precision_knn, recall_knn,
                                                                           f1_knn, specificity_knn, g_mean_knn)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'ridge':
        ...
        for n in range(len(percentile_threshold)):
            ...
            if classifier_type == 'all' or classifier_type == 'KNN':
                accuracy_knn, precision_knn, recall_knn, f1_knn, specificity_knn, g_mean_knn = (
                    classify_knn(x_train_ridge[n], x_test_ridge[n], y_train, y_test, random_state,
                                 save_folder=os.path.join("../ModelSave/SequentialOptimizationFeatureReduction",
                                                          classifier_type,
                                                          timestamp, method_key), retrain=train_class))

            performance_metric_summary[n] = single_classifier_performance_summary(accuracy_knn, precision_knn, recall_knn,
                                                                               f1_knn, specificity_knn, g_mean_knn)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'mrmr':
        ...
        if classifier_type == 'all' or classifier_type == 'KNN':
            accuracy_knn, precision_knn, recall_knn, f1_knn, specificity_knn, g_mean_knn = (
                classify_knn(x_train_mrmr, x_test_mrmr, y_train, y_test, random_state,
                             save_folder=os.path.join("../ModelSave/SequentialOptimizationFeatureReduction",
                                                      classifier_type,
                                                      timestamp, method_key), retrain=train_class))
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'none':
        ...
        performance_metric_summary = single_classifier_performance_summary(accuracy_knn, precision_knn, recall_knn, f1_knn, specificity_knn, g_mean_knn)
