...
def main_loop(impute_type, n_neighbors, timestamp, model_type, method_key, baseline_window, window_size, stride, classifier_type,
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'lasso':
        ...
        if classifier_type == 'all' or classifier_type == 'logreg':
            ...
            performance_metric_summary = single_classifier_performance_summary(accuracy_logreg, precision_logreg,
                                                                               recall_logreg, f1_logreg,
                                                                               specificity_logreg, g_mean_logreg)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'ridge':
        ...
        for n in range(len(percentile_threshold)):
            ...
            if classifier_type == 'all' or classifier_type == 'logreg':
                accuracy_logreg, precision_logreg, recall_logreg, f1_logreg, specificity_logreg, g_mean_logreg = (
                    classify_logistic_regression(x_train_ridge[n], x_test_ridge[n], y_train, y_test, class_weight_imb, random_state,
                                                 save_folder=os.path.join(
                                                     "../ModelSave/SequentialOptimizationFeatureReduction",
                                                     classifier_type,
                                                     timestamp, method_key), retrain=train_class))

                performance_metric_summary[n] = single_classifier_performance_summary(accuracy_logreg, precision_logreg,
                                                                                   recall_logreg, f1_logreg,
                                                                                   specificity_logreg, g_mean_logreg)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'mrmr':
        ...
        if classifier_type == 'all' or classifier_type == 'logreg':
            accuracy_logreg, precision_logreg, recall_logreg, f1_logreg, specificity_logreg, g_mean_logreg = (
                classify_logistic_regression(x_train_mrmr, x_test_mrmr, y_train, y_test, class_weight_imb, random_state,
                                             save_folder=os.path.join(
                                                 "../ModelSave/SequentialOptimizationFeatureReduction",
                                                 classifier_type,
                                                 timestamp, method_key), retrain=train_class))
