...
def main_loop(impute_type, n_neighbors, timestamp, model_type, method_key, baseline_window, window_size, stride, classifier_type,
        ...
        if classifier_type == 'all' or classifier_type == 'rf':
            accuracy_rf, precision_rf, recall_rf, f1_rf, tree_depth, specificity_rf, g_mean_rf = (
                classify_random_forest(x_train, x_test, y_train, y_test, class_weight_imb, random_state,
                             save_folder=os.path.join("../ModelSave/SequentialOptimizationFeatureReduction", classifier_type, timestamp,
                                                      method_key), retrain=train_class))

        performance_metric_summary = single_classifier_performance_summary(accuracy_rf, precision_rf, recall_rf, f1_rf, specificity_rf, g_mean_rf)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'ridge':
        ...
        for n in range(len(percentile_threshold)):
            ...
            if classifier_type == 'all' or classifier_type == 'rf':
                accuracy_rf, precision_rf, recall_rf, f1_rf, tree_depth, specificity_rf, g_mean_rf = (
                    classify_random_forest(x_train_ridge[n], x_test_ridge[n], y_train, y_test, class_weight_imb, random_state,
                                           save_folder=os.path.join(
                                               "../ModelSave/SequentialOptimizationFeatureReduction", classifier_type, timestamp,
                                               method_key), retrain=train_class))

            performance_metric_summary[n] = single_classifier_performance_summary(accuracy_rf, precision_rf, recall_rf,
                                                                               f1_rf, specificity_rf, g_mean_rf)
    # Minimum Redundancy Maximum Relevance | mrmr
    if feature_reduction_type == 'all' or feature_reduction_type == 'mrmr':

        # First reduce predictors using ridge regression
        x_train_mrmr_step1, x_test_mrmr_step1, selected_features_mrmr_step1 = feature_selection_ridge(x_train, x_test,
                                                                                                      y_train,
                                                                                                      all_features,
                                                                                                      30,
                                                                                                      random_state)
        ...
        number_features = np.round(0.2 * len(selected_features_mrmr_step1))

        # Determine reduced feature set & transform x_train and x_test with the default number of features
        x_train_mrmr, x_test_mrmr, selected_features = feature_selection_mrmr(x_train_mrmr_step1, y_train,
                                                                             x_test_mrmr_step1, selected_features_mrmr_step1,
                                                                             number_features)

        # Random Forrest | rf
        if classifier_type == 'all' or classifier_type == 'rf':
            accuracy_rf, precision_rf, recall_rf, f1_rf, tree_depth, specificity_rf, g_mean_rf = (
                classify_random_forest(x_train_mrmr, x_test_mrmr, y_train, y_test, class_weight_imb, random_state,
                                       save_folder=os.path.join(
                                           "../ModelSave/SequentialOptimizationFeatureReduction", classifier_type, timestamp,
                                           method_key), retrain=train_class))

        performance_metric_summary = single_classifier_performance_summary(accuracy_rf, precision_rf, recall_rf,
                                                                           f1_rf, specificity_rf, g_mean_rf)
    ...
    if feature_reduction_type == 'all' or feature_reduction_type == 'none':
        ...
        if classifier_type == 'all' or classifier_type == 'rf':
            accuracy_rf, precision_rf, recall_rf, f1_rf, tree_depth, specificity_rf, g_mean_rf = (
                classify_random_forest(x_train, x_test, y_train, y_test, class_weight_imb, random_state,
                                       save_folder=os.path.join("../ModelSave/SequentialOptimizationFeatureReduction", classifier_type, 
                                                                timestamp, method_key), retrain=train_class))
...
if __name__ == "__main__":
    ...
    feature_reduction_methods = ['lasso', 'enet', 'ridge', 'pca', 'performance', 'none']
    feature_reduction_methods = ['performance', 'none']
    ...
    # Save pkl summary
    save_folder = os.path.join("../PerformanceSave/SequentialOptimizationFeatureReduction_reduced_baseline", classifier_to_use, timestamp_outerloop)
    save_file = 'Sequential_Optimization_Feature_Reduction_dictionary_model_type_' + str(model_type[1]) + '_reduced_baseline.pkl'
