...
def make_objective(x_train, y_train, class_weights, random_state, save_folder, use_sampler, objective_var, all_features):
    ...
    def objective(trial):
        ...
        if best_stopping_metric >= best_value:
            ...
            # Save model weights
            torch.save(model.state_dict(), os.path.join(save_folder, "Trans_best_model.pt"))

            # Save hyperparameters
            with open(os.path.join(save_folder, "Trans_best_params.json"), "w") as f:
            ...
            with open(os.path.join(save_folder, "Trans_best_trial_metadata.json"), "w") as f:
...
def transformer_class(x_train, x_test, y_train, y_test, class_weight_imb, random_state, all_features, save_folder):
    ...
    study_path = os.path.join(save_folder, "Trans_optuna_study.pkl")
