...
def main_loop(impute_type, n_neighbors, timestamp, model_type, method_key, baseline_window, window_size, stride):
    ...
    feature_matrix_name = ('x_feature_matrix_sliding_window_baseline_' + str(round(current_baseline_window, 0)) +
                           '_window_size_' + str(round(current_window_size, 0)) +
                           '_stride_' + str(round(current_stride, 0)) + '_model_type_' + str(model_type[1]) +'.pkl')
    y_label_name = ('y_gloc_labels_sliding_window_baseline_' + str(round(current_baseline_window, 0)) +
                    '_window_size_' + str(round(current_window_size, 0)) + '_stride_' + str(round(current_stride, 0)) +
                    '_model_type_' + str(model_type[1]) +'.pkl')
    all_features_name = ('all_features_sliding_window_baseline_' + str(round(current_baseline_window, 0)) +
                         '_window_size_' + str(round(current_window_size, 0)) + '_stride_' + str(round(current_stride, 0)) +
                         '_model_type_' + str(model_type[1]) +'.pkl')
...
############################################# LOAD AND PROCESS DATA #############################################
 ...
 ################################################ MACHINE LEARNING ################################################
    ...
    if classifier_type == 'all' or classifier_type == 'logreg':
        ...
        performance_metric_summary = single_classifier_performance_summary(accuracy, precision, recall, f1, specificity, g_mean)
    ...
    for i in range(len(baseline_windows_to_analyze)):
        ...
        for j in range(len(window_sizes_to_analyze)):
            ...
            for k in range(len(strides_to_analyze)):
                ...
                sliding_window_performance_summary[method_key_sliding_window] = main_loop(impute_type_all[i], n_neighbors_all[j],
                                                                                          timestamp_outerloop, model_type_outerloop,
                                                                                          sliding_window_performance_summary,
                                                                                          current_baseline_window,
                                                                                          current_window_size,
                                                                                          current_stride)

    # Save pkl summary
    save_folder = os.path.join("../PerformanceSave/SequentialOptimizationSlidingWindow", timestamp_outerloop)
