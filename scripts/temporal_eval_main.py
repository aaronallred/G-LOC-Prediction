...
if preference == 3:
	...
	for m in range(len(classifiers_to_test)):
		...
		splits_dir = os.path.join(save_folder, "split_indices")
		os.makedirs(splits_dir, exist_ok=True)
		split_indices_path = os.path.join(splits_dir, f"split_indices_{classifier}.pkl")

		if use_saved_splits:
			if not os.path.exists(split_indices_path):
				raise FileNotFoundError(
					f"Saved split indices not found at {split_indices_path}. "
					f"Run with use_saved_splits=False and enable_imputation=True first."
				)
		...
		hyperparameters, select_features, foldID_check, score_check = get_hyperparameters_from_json(
			classifier, get_model_subfolder(model_type)
		)
		...
		for i in range(len(offset_ranges)):
			...
			(x, y, survivor_indices) = data_with_prediction(
				offset_ranges[i],
				data_rate,
				classifier,
				model_type,
				select_features,
				impute=enable_imputation,
				return_indices=True,
			)
			...
			for k in range(num_kfold):
				...
				if use_saved_splits:
					saved = split_indices[(offset_ranges[i], k)]
					x_train, x_test, y_train, y_test = split_by_saved_indices(
						ravel(y), x, survivor_indices, saved["train"], saved["test"]
					)
				else:
					x_train, x_test, y_train, y_test, train_idx, test_idx = stratified_kfold_split(
						ravel(y), x, num_kfold, k, random_state, return_indices=True
					)
					split_indices[(offset_ranges[i], k)] = {
						"train": survivor_indices[train_idx],
...
if preference == 4:
	...
	window_lengths = {"logreg": 12.5, "RF": 7.5, "LDA": 15, "SVM": 15, "EGB": 12.5, "KNN": 15}
	...
	plot_f1_scores_across_classifiers(
		f1_score_dict, window_lengths, get_model_subfolder(model_type), shared_plot=True
	)
...
if preference == 7:
	...
	# SVM
	classifier = "SVM"
	_, select_featuresLDA, _, _ = get_hyperparameters_from_json(
		classifier, get_model_subfolder(model_type)
	)

	# KNN
	classifier = "KNN"
	_, select_featuresKNN, _, _ = get_hyperparameters_from_json(
		classifier, get_model_subfolder(model_type)
	)
	...
	for classifier in classifiers_to_test:
		...
		for i in range(len(offset_ranges)):
			x, y = data_with_prediction(
				offset_ranges[i], data_rate, classifier, model_type, select_features
			)
		...
		subfolder2 = (
			"FeatureShare"  # Adding folder level for looking at performance when limiting features
		)
		plotting_offset_models(
			offset_ranges,
			accuracy_model,
