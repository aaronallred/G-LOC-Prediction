...
from imputation import eeg_condition_impute, faster_knn_impute
from prediction import process_NaN_temporal, y_prediction_offset


def data_with_prediction(
	backstep,
	data_rate,
	classifier_type,
	model_type,
	...
	feature_matrix_name = os.path.join(
		cache_folder, f"x_feature_matrix_method_{classifier_type}.pkl"
	)
	...
	if backstep == 0:
		...
		(
			...
			all_features_ecg,
			all_features_eeg,
		) = analysis_driven_csv_processing(
			analysis_type,
			filename,
			feature_groups_to_analyze,
			demographic_data_filename,
		...
		if "complete" in model_type and "explicit" in model_type:
			...
			gloc_data_reduced, features, features_phys, features_eeg = eeg_condition_impute(
				gloc_data_reduced,
				all_features,
				all_features_phys,
				all_features_eeg,
			...
			gloc_data_reduced["AFE_indicator"] = (
				afe_indicator_column  # Merge afe_indicators back into the predictor set
			)
		...
		if "complete" in model_type and "explicit" in model_type:
			afe_indicator_column = (
				gloc_data_reduced["AFE_indicator"].to_numpy(dtype=np.float32).reshape(-1, 1)
			)

		del gloc_data_reduced

		save_variables_to_folder(
			cache_folder,
			{
				"gloc": gloc,
				...
				"indicator_afe": afe_indicator_column
				if "complete" in model_type and "explicit" in model_type
				else None,
			},
		)
		...
		# Load from cache
		cached_vars = load_variables_from_folder(
			cache_folder,
			[
				"gloc",
				"trial_column",
	...
	gloc = y_prediction_offset(
		gloc, backstep, data_rate, trial_column
	)  # Call function to shift gloc flags around
	...
	if backstep == 0:
		combined_baseline, combined_baseline_names, baseline_v0, baseline_names_v0 = baseline_data(
			baseline_methods_to_use,
			trial_column,
			time_column,
			event_validated_column,
		...
		cached_vars2 = load_variables_from_folder(
			cache_folder,
			[
				# existing variables...
				"combined_baseline",
	...
	if backstep == 0:
		...
		x_feature_matrix, select_features = remove_constant_columns(
			x_feature_matrix, select_features
		)
		...
		save_variables_to_folder(
			cache_folder, {"removed_ind": removed_ind, "surviving_indices": surviving_indices}
		)
	...
	if backstep == 0:
		...
		print(
			f"Saved all_features to {all_features_name}, size={os.path.getsize(all_features_name)} bytes"
		)
		...
		print(
			f"Saved x_features to {feature_matrix_name}, size={os.path.getsize(feature_matrix_name)} bytes"
		)
...
def plotting_offset_models(
	...
	for idx, (label, matrix, color) in enumerate(metrics, start=1):
		...
		plt.fill_between(
			offsets, min_vals, max_vals, color="gray", alpha=0.3, label="Range (min–max)"
		)

		# Add vertical dashed line at window_size
		plt.axvline(
			x=window_size, color="black", linestyle="--", linewidth=1.5, label="Window Size"
		)
		...
		plt.legend()
		plt.suptitle(
			f"Metrics Across Offsets — Classifier: {classifier_name} {get_model_subfolder(model_type)}",
			fontsize=16,
			fontweight="bold",
		)
...
def plot_metrics_from_cache(classifier_name, model_type):
	...
	window_sizes = {"logreg": 12.5, "RF": 7.5, "LDA": 15, "SVM": 15, "EGB": 12.5, "KNN": 15}
	...
	plt.suptitle(
		f"Metrics Across Offsets — Classifier: {classifier_name} {model_name}",
		fontsize=16,
		fontweight="bold",
	)
	plt.tight_layout()
...
def plot_f1_scores_across_classifiers(
	f1_score_dict, window_lengths, model_type_name, shared_plot=False
):
	"""
	...
	if shared_plot:
		...
		plt.title(
			f"F1 Score Across Offsets for All Classifiers — {model_type_name}",
			fontsize=18,
			fontweight="bold",
		)
		plt.tight_layout()
...
def get_hyperparameters_from_json(classifier: str, model_type: str):
	...
	json_path = os.path.join(
		BASE_DIR, "ModelSave", "CV", model_type, f"median_hyperparameters_{classifier}.json"
	)
...
def get_median_hyperparameters(classifier: str, model_type: str):
	...
	if classifier == "RF":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "random_forest_model.pkl"
		)
	elif classifier == "KNN":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "KNN_model.pkl"
		)
	elif classifier == "EGB":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "ensemble_model.pkl"
		)
	elif classifier == "logreg":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "logistic_regression_model.pkl"
		)
	elif classifier == "SVM":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "SVM_model.pkl"
		)
	elif classifier == "LDA":
		model_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "LDA_model.pkl"
		)

	# Default feature path
	features_path = os.path.join(
		BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, f"SelectedFeatures{classifier}.pkl"
	)

	# Logistic regression uses a different filename convention
	if classifier == "logreg":
		features_path = os.path.join(
			BASE_DIR, "ModelSave", "CV", model_type, median_fold_id, "SelectedFeaturesLR.pkl"
		)
