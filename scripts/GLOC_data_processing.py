...
import warnings
#from GLOC_main import learningMode
...
import numpy as np
# from learningMode import GLOC_main.py

def process_NaN(y_gloc_labels, x_feature_matrix, all_features, trials=None):

    nan_cols = np.all(np.isnan(x_feature_matrix), axis=0)
    x_clean_cols = x_feature_matrix[:, ~nan_cols]
    features_clean = [f for i, f in enumerate(all_features) if not nan_cols[i]]

    # 2) drop any rows containing NaN
    row_mask = ~np.isnan(x_clean_cols).any(axis=1)
    y_clean = y_gloc_labels[row_mask]
    x_clean = x_clean_cols[row_mask]

    # 3) include trials if provided
    if trials is not None:
        trials_clean = trials[row_mask]
        return y_clean, x_clean, features_clean, trials_clean

    # default: no trials returned
    return y_clean, x_clean, features_clean
...
# Rename, as of now has nicoles naming conventions, look into Sequential_Optimization files
