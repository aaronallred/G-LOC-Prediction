...
def plot_tgt_pos(df, time_variable, trial_id):
    """
    Plots the X/Y target positions for one trial.

    Parameters
    ----------
    df : pandas.DataFrame
        Must contain columns ['trial_id', time_variable, 'tgtposX - Cog', 'tgtposY - Cog'].
    time_variable : str
        Name of the time column (e.g. 'Time (s)').
    trial_id : str
        The trial identifier to plot (e.g. '01-01').
    """
    # mask down to the single trial
    mask = (df['trial_id'] == trial_id)
    times = df.loc[mask, time_variable].values
    x_vals = df.loc[mask, 'tgtposX - Cog'].values
    y_vals = df.loc[mask, 'tgtposY - Cog'].values

    fig, ax = plt.subplots()
    ax.plot(times, x_vals, label='Target X')
    ax.plot(times, y_vals, label='Target Y')

    ax.set_xlabel(time_variable)
    ax.set_ylabel('Target Position (pixels)')
    ax.set_title(f'Target Position for Trial {trial_id}')
    ax.legend()
    plt.show()



def plot_strain(df, time_variable='Time (s)', centrifuge_col='magnitude - Centrifuge'):
    trial_ids = df['trial_id'].unique()
    for tid in trial_ids:
        idx = df['trial_id']==tid
        t = df.loc[idx, time_variable]
        strain = df.loc[idx, 'Strain [0/1]']
        cen_g = df.loc[idx, centrifuge_col]
        plt.figure(); plt.plot(t, strain, label='Strain')
        plt.plot(t, cen_g, label='centrifuge g')
