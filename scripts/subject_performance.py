...
def main_analysis_loop(filename, plot_data, plot_pairwise, feature_to_analyze, time_variable, analysis_type,
    ...
    # Logistic Regression
    accuracy_logreg, precision_logreg, recall_logreg, f1_logreg = classify_logistic_regression(y_gloc_labels_noNaN,
                                                                                               x_feature_matrix_noNaN,
                                                                                               training_ratio,
                                                                                               all_features)

    # RF
    accuracy_rf, precision_rf, recall_rf, f1_rf = classify_random_forest(y_gloc_labels_noNaN, x_feature_matrix_noNaN,
                                                                         training_ratio, all_features)

    # LDA
    accuracy_lda, precision_lda, recall_lda, f1_lda = classify_lda(y_gloc_labels_noNaN, x_feature_matrix_noNaN,
                                                                   training_ratio, all_features)

    # KNN
    accuracy_knn, precision_knn, recall_knn, f1_knn = classify_knn(y_gloc_labels_noNaN, x_feature_matrix_noNaN,
                                                                   training_ratio)

    # SVM
    accuracy_svm, precision_svm, recall_svm, f1_svm = classify_svm(y_gloc_labels_noNaN, x_feature_matrix_noNaN,
                                                                   training_ratio)

    # Ensemble with Gradient Boosting
    accuracy_gb, precision_gb, recall_gb, f1_gb = classify_ensemble_with_gradboost(y_gloc_labels_noNaN,
                                                                                   x_feature_matrix_noNaN,
                                                                                   training_ratio)
