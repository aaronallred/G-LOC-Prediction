...
def main_loop(kfold_ID, num_splits, param_path, impute_path, horizons, save_folder, classifier_type, model_type):
    ...
    train_class = False  # not yet set up to test and not train (always trains)
        ...
        for horizon in horizons:
            accuracy, precision, recall, f1, specificity, g_mean = (
                transformer_class_load(
                    x_train, x_test, y_train, y_test, horizon, class_weight_imb, random_state,
                    all_features, param_path=param_path, save_folder=save_folder, load_weights=(not train_class)
                )
            )
...
if __name__ == "__main__":
    ...
    summary_loc = os.path.join("../PerformanceSave/LoadAndEval",subFolder)
    ...
    save_file = f"{classifier_type}_{'_'.join(model_type)}_eval.pkl"
