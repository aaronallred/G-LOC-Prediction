...
def make_objective(x_train, y_train, class_weights, random_state, save_folder, use_sampler, objective_var, all_features):
    ...
    def objective(trial):
        ...
        if best_stopping_metric >= best_value:
            ...
            # Save model weights
            torch.save(model.state_dict(), os.path.join(save_folder, "LSTM_best_model.pt"))

            # Save hyperparameters
            with open(os.path.join(save_folder, "LSTM_best_params.json"), "w") as f:
            ...
            with open(os.path.join(save_folder, "LSTM_best_trial_metadata.json"), "w") as f:
...
def lstm_binary_class(x_train, x_test, y_train, y_test, class_weight_imb, random_state, all_features, save_folder):
    ...
    study_path = os.path.join(save_folder, "LSTM_optuna_study.pkl")
...
def lstm_binary_class_load(x_train, x_test, y_train, y_test, horizon, class_weight_imb, random_state, all_features,
    ...
    params_path = os.path.join(param_path, "LSTM_best_params.json")
    with open(params_path, "r") as f:
        best_params = json.load(f)

    metadata_path = os.path.join(param_path, "LSTM_best_trial_metadata.json")
    ...
    print("\nPerformance Metrics for LSTM (from JSON):")
    print("Accuracy: ", accuracy)
    print("Precision: ", precision)
    print("Recall: ", recall)
    print("F1 Score: ", f1)
