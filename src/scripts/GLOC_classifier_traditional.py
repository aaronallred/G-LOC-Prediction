...
def classify_random_forest(x_train, x_test, y_train, y_test, class_weight_imb, random_state, save_folder,
                           model_name, retrain, temporal = False, best_params = None, select_features = None, feat_imp = False):
    """
    ...
    if feat_imp:
        explanation = run_shap(rf, x_train, x_test, select_features, random_state)
    else:
        explanation = None
    ...
    return accuracy, precision, recall, f1, tree_depth, specificity, g_mean, explanation
...
# k Nearest Neighbors
def classify_knn(x_train, x_test, y_train, y_test, random_state, save_folder,model_name,
                 retrain, temporal = False, best_params = None , select_features = None, feat_imp = False):
    """
    ...
    if feat_imp:
        explanation = run_shap(neigh, x_train, x_test, select_features, random_state)
    else:
        explanation = None

    return accuracy, precision, recall, f1, specificity, g_mean, explanation
...
def classify_ensemble_with_gradboost(x_train, x_test, y_train, y_test, random_state, save_folder,
                                     model_name, retrain, temporal = False, best_params = None, select_features = None, feat_imp = False):
    """
    ...
    if feat_imp:
        explanation = run_shap(gb, x_train, x_test, select_features, random_state)
    else:
        explanation = None
