...
EVENT_COL = "event"
EVENT_VALIDATED_COL = "event_validated"
TRIAL_ID_COL = "trial_id"
SUBJECT_COL = "subject"
MAGNITUDE_COL = "magnitude - Centrifuge"
STRAIN_LABEL = "strain during GOR"
END_GOR_LABEL = "end GOR"
MAX_GOR_WINDOW = 5000
BASELINE_G = 1.2
...
DEMOGRAPHIC_SOURCE_COLUMNS = [
    "Gender code [1/0]", "Age [yr]", "height [m]", "weight (kg)",
    "BMI [kg/m^2]", "Blood Volume [L]", "Resting SBP (seat)",
    "resting SBP (stand)", "SBP after squat", "Resting DBP (seat)",
    "resting DBP (stand)", "DBP after squat", "Resting MAP",
    "Resting MAP (stand", "Post-Squat MAP", "resting HR [seated]",
    "resting HR (stand)", "HR after squat", "Max (N)",
    "largest leg circ. [cm]", "lower leg volume [mL]", "chest avg",
    "abd avg", "thigh avg", "midax avg", "subscap avg", "tri avg",
    "supra avg", "sum", "% fat", "leg avg", "arm avg",
    "neck (MNL) avg", "neck (LNL) avg", "torso (post) avg ",
    "torso (ax) avg ", "head to heart avg", "head avg", "neck avg",
    "chest upper avg", "chest under avg", "waist avg", "hip avg",
    "thigh avg", "calf avg", "bicep flex avg", "bicep relax avg",
    "avg (N) flexion", "avg (N) extens", "avg (N) Rt. Rot",
    "avg (N) left rot", "avg (N) left lat flex", "avg (N) rt lat flex",
    "pred. Vo2"
]

DEMOGRAPHIC_NAMES = [
    "participant_gender", "participant_age", "participant_height", "participant_weight",
...
STRAIN_RULES = [
    ("04-06", 6.1, False, False),
    ("06-02", 8.1, False, False),
    ("07-06", 4.6, False, False),
    ("08-02", 8.3, False, False),
    ("08-05", 4.3, True, False),
    ("08-06", 8.2, False, False),
    ("09-03", 8.7, False, False),
    ("09-05", 4.8, False, False),
    ("10-05", 3.8, False, False),
    ("10-06", 5.5, False, True),
    ("11-02", 5.5, False, False),
    ("12-03", 3.7, False, False),
    ("13-02", 7.3, False, False),
    ("13-04", 7.4, False, False),
]

END_GOR_ONLY_TRIALS = ["06-06"]
...
class GGroup(BaseFeatureGroup):
    def get_feature_names(self, model_type):
        if model_type.feature_set == "Implicit":
            ...
            warnings.warn("G cannot be used as a feature in implicit models. Feature removed.", stacklevel=2)
    ...
    def process(self, df, file_paths):
        ...
        df[MAGNITUDE_COL] = df[MAGNITUDE_COL].fillna(BASELINE_G)
...
class CognitiveGroup(BaseFeatureGroup):
    ...
    def process(self, df, file_paths):
        ...
        df["Correct - Cog"] = df["Correct - Cog"].replace({
            "correct": 1,
            "no response": 0,
            "incorrect": -1,
            "NO VALUE": np.nan
        ...
        warnings.warn("Per information from Chris on 03/12/25, Cognitive data only collected right before and right after ROR.", stacklevel=2)
...
class RawEEGGroup(BaseFeatureGroup):
    _SEPARATED_FEATURES = {
        "Shared": [
            "Fz - EEG", "F3 - EEG", "C3 - EEG", "C4 - EEG", "CP1 - EEG",
            "CP2 - EEG", "T8 - EEG", "TP9 - EEG", "TP10 - EEG", "P7 - EEG", "P8 - EEG"
        ],
        "AFE Only": ["F4 - EEG", "T7 - EEG", "O1 - EEG", "O2 - EEG"],
        "Non-AFE Only": [
            "F1 - EEG", "AFz - EEG", "AF4 - EEG", "FT9 - EEG", "FT10 - EEG", "FC5 - EEG",
    ...
    def get_feature_names(self, model_type):
        raw_eeg_features = self._SEPARATED_FEATURES
    ...
    def get_separated_feature_names():
        return RawEEGGroup._SEPARATED_FEATURES
...
class ProcessedEEGGroup(BaseFeatureGroup):
    ...
    def get_feature_names(self, model_type):
        processed_eeg_features = self._SEPARATED_FEATURES
    ...
    @staticmethod
    def get_separated_feature_names():
        return ProcessedEEGGroup._SEPARATED_FEATURES

class StrainGroup(BaseFeatureGroup):
    def get_feature_names(self, model_type):
        if model_type.feature_set == "Implicit":
            # Output warning message for implicit vs. explicit models
            warnings.warn("Strain cannot be used as a feature in implicit models. Feature removed.", stacklevel=2)
    ...
    def process(self, df, file_paths):
        ...
        event_validated = df[EVENT_VALIDATED_COL].to_numpy()
        strain_event = np.zeros(event.shape)

        # Find labeled 'strain' and 'end GOR' markings in the event column
        strain_indices = np.argwhere(event == STRAIN_LABEL)
        end_GOR_indices = np.argwhere(event_validated == END_GOR_LABEL)
        ...
        end_gor_lookup = {}
        for trial_code in trial_strain.drop_duplicates():
            contains_trial = trial_end_gor.str.contains(trial_code, regex=False)
            if contains_trial.any():
                end_gor_lookup[trial_code] = contains_trial[contains_trial].index.tolist()[0]
        ...
        for i in range(trial_strain.shape[0]):
            end_gor_trial_index = end_gor_lookup.get(trial_strain.iloc[i])
            if end_gor_trial_index is not None:
                strain_event[strain_indices[i, 0]:end_gor_trial_index] = 1
    ...
    def add_missing_strain(self, gloc_data_reduced):
        # Add missing strain during GOR labels based on GLOC Effectiveness Spreadsheet
        gloc_trial = gloc_data_reduced[TRIAL_ID_COL]
        magnitude_g = gloc_data_reduced[MAGNITUDE_COL].to_numpy()

        for trial_individual_coding in END_GOR_ONLY_TRIALS:
            trial_index = gloc_data_reduced[TRIAL_ID_COL] == trial_individual_coding
            magnitude_g_trial = magnitude_g[trial_index]
            gor_peak = np.nanargmax(magnitude_g_trial[0:MAX_GOR_WINDOW])
            self._add_end_gor_label_if_available(gloc_data_reduced, trial_index, magnitude_g_trial, gor_peak)

        for trial_individual_coding, g_level_strain, add_end_gor, clear_existing_strain in STRAIN_RULES:
            trial_index = gloc_data_reduced[TRIAL_ID_COL] == trial_individual_coding
            magnitude_g_trial = magnitude_g[trial_index]
            current_event = gloc_data_reduced[EVENT_COL][trial_index]

            if clear_existing_strain:
                mislabel_mask_strain = current_event.str.contains(STRAIN_LABEL)
                gloc_data_reduced.loc[current_event[mislabel_mask_strain].index, EVENT_COL] = None
            ...
            gloc_data_reduced.loc[strain_label_index, EVENT_COL] = STRAIN_LABEL
    ...
    @staticmethod
    def _add_end_gor_label_if_available(gloc_data_reduced, trial_index, magnitude_g_trial, gor_peak):
        return_to_base_spin_candidates = np.where(magnitude_g_trial[gor_peak:-1] == BASELINE_G)[0]
        if len(return_to_base_spin_candidates) > 0:
            ...
            gloc_data_reduced.loc[end_gor_label_index, EVENT_VALIDATED_COL] = END_GOR_LABEL
    
class DemographicsGroup(BaseFeatureGroup):
    def get_feature_names(self, model_type):
        if model_type.feature_set == "Implicit":
            # Output warning message for implicit vs. explicit models
            warnings.warn("Demographics cannot be used as features in implicit models. Features removed.", stacklevel=2)
    ...
    def read_and_process_demographics(self, demographic_data_filename, gloc_data_reduced):
        ...
        float64_cols = demographics.select_dtypes(include="float64").columns
        if len(float64_cols) > 0:
            demographics = demographics.astype({col: "float32" for col in float64_cols})

        all_demographics = demographics[DEMOGRAPHIC_SOURCE_COLUMNS]

        # Initialize variables
        participant_demographics = np.zeros((len(gloc_data_reduced), all_demographics.shape[1]))
        length_previous_participant_data = -1
        subject_values = gloc_data_reduced[SUBJECT_COL].to_numpy()

        # Organize all participant data in array same length as gloc_data_reduced
        for i, participant in enumerate(PARTICIPANT_LIST):
            length_current_participant_data = np.count_nonzero(subject_values == participant)
            participant_demographics[
                length_previous_participant_data + 1:length_previous_participant_data + 1 + length_current_participant_data, :
            ] = all_demographics.iloc[i].to_numpy()
        ...
        demographics_concat = pd.DataFrame(participant_demographics, columns=DEMOGRAPHIC_NAMES)

        # Append all demographic data to gloc data reduced
        gloc_data_reduced = pd.concat([gloc_data_reduced, demographics_concat], axis=1)
        return gloc_data_reduced, DEMOGRAPHIC_NAMES
