...
class AdvancedDataPipeline(DataPipeline):
    ...
    * Implicit modes show higher memory usage, likely due to intermediate array allocation
    ...
    FEATURE_GROUPS_BY_MODEL_TYPE = {
        ("noAFE", "Explicit"): ("ECG", "BR", "temp", "eyetracking", "G", "rawEEG", "processedEEG", "strain", "demographics"),
        ("noAFE", "Implicit"): ("ECG", "BR", "temp", "eyetracking", "rawEEG"),
        ("Complete", "Explicit"): ("ECG", "BR", "temp", "eyetracking", "G", "rawEEG", "processedEEG", "strain", "demographics"),
        ("Complete", "Implicit"): ("ECG", "BR", "temp", "eyetracking", "rawEEG"),
    }
    BASELINING_CHARACTERISTICS_BY_MODEL_TYPE = {
        "noAFE": ["v0", "v1", "v2", "v5", "v6", "v7", "v8"],
        "Complete": ["v0", "v1", "v2", "v5", "v6"],
    ...
    def get_data(
        ...
        Parameters:
            model_type: ModelType — (afe_filter, feature_set) e.g. ('Complete', 'Explicit')
        ...
        ############################################# LOAD AND PROCESS DATA #############################################
        data_locations = self._get_data_locations()
        gloc_data = self._load_data(data_locations)
        gloc_data = self._filter_data_by_analysis_type(analysis_type, gloc_data, subject_to_analyze, trial_to_analyze)
        gloc_data, features = self._process_and_get_feature_names(gloc_data, feature_groups_to_analyze, model_type, data_locations)
    ...
    def _get_feature_groups_and_baseline_methods(self, model_type: Tuple[str, str]) -> Tuple[Sequence[str], List[str]]:
        """ Determine which feature groups and baseline methods to use based on the specified model type. """
